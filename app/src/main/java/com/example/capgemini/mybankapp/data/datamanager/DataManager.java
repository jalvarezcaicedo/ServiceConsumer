...
public class DataManager {
    ...
    public Observable<Response<Void>> callLogin(Context context, Login login) throws CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException {
    ...
    public Observable<Response<List<CustomerProductResponse>>> callCustomerProducts(Context context) throws CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException {

        SharedPreferences sharedPreferences = context.getSharedPreferences(Constants.SHARED_PREFERENCES, Context.MODE_PRIVATE);

        String url = "/api/1.0/product/" + sharedPreferences.getString(Constants.CUSTOMER_ID, "") + "/get";
        //String authorization = sharedPreferences.getString(Constants.AUTHORIZATION_HEADER, "");

        return TransactionalService.newTransactionalService(context).customerProductsCall(url);
    }

    public Observable<Response<CustomerProductResponse>> callCustomerProductInfo(Context context, String productNumber) throws CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException {
        ...
        return TransactionalService.newTransactionalService(context).customerProductInfoCall(url, authorization);
    }
    ...
    public Observable<Response<List<TransactionResponse>>> callTransaction(Context context, String limit, String productNumber) throws CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException {
        ...
        String url = "/api/1.0/transaction/getLast/" + limit + "/" + sharedPreferences.getString(Constants.CUSTOMER_ID, "") + "/" + productNumber;
        String authorization = sharedPreferences.getString(Constants.AUTHORIZATION_HEADER, "");

        return TransactionalService.newTransactionalService(context).lastTransactionsCall(url, authorization);
    }

    public Observable<Response<TransactionResponse>> callSaveTransaction(Context context, TransactionRequest transactionRequest) throws CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException, IOException {

        SharedPreferences sharedPreferences = context.getSharedPreferences(Constants.SHARED_PREFERENCES, Context.MODE_PRIVATE);

        String authorization = sharedPreferences.getString(Constants.AUTHORIZATION_HEADER, "");

        return TransactionalService.newTransactionalService(context).saveTransactionCall(authorization, transactionRequest);
    }
